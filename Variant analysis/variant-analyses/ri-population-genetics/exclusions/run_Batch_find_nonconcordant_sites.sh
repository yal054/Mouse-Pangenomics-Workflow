...
# /path/to/find_nonconcordant_sites.py: script included in this repository at `Variant analysis/variant-analyses/ri-population-genetics/exclusions/find_nonconcordant_sites.py`.
#SBATCH --mem=8G
#SBATCH -n 1
#SBATCH -N 1
...
start_time=$(date +%s)
echo "Running script..."
python3 /path/to/find_nonconcordant_sites.py \
  --vcf "$VCF" \
  --sample "$SAMPLE" \
  --hapl "$HAPL" \
  --out "$OUT"
